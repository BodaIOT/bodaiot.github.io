...
// ---------- RGB <-> HSL ----------
// HSL is converted straight from RGB (the same definition CSS hsl() uses)
// rather than being derived from the HSV state, so the value shown always
// round-trips back to the exact color in the swatch.
// h is in degrees (0..360), s and l in percent (0..100).
...
function rgbToHsl(r, g, b) {
  ...
  return { h, s: clamp(s, 0, 1) * 100, l: l * 100 };
}
...
function updateFields(rgb) {
  ...
  setIfNotFocused(hslInput, `${Math.round(hsl.h % 360)}, ${Math.round(hsl.s)}, ${Math.round(hsl.l)}${alphaSuffix}`);
}
...
hslInput.addEventListener('input', () => {
  const n = parseNums(hslInput.value);
  if (!n || n.length < 3) { render(); return; }
...
[hexInput, rgb255Input, rgb1Input, rgba255Input, rgba1Input, hsvInput, cmykInput, hslInput].forEach((input) => {
